 ...
 * Some useful macros
 *
 * ------ MAPPING OF DIOs for ETHZ LoRa/WuRx platform (June 2016 version)
 * DIO0 - 1.3
 * DIO1 - NC
 * DIO2 - NC
 * DIO3 - NC
		...
		event_handler_t timeout_handler, event_handler_t crc_err_handler)
{
	...
	P1IES &= ~BIT3;	// Interrupt on a low to high transition
...
void sx1276_tx_pkt(char *data, uint8_t pkt_size, uint8_t address)
{
	...
	else if ((_modem == MODEM_OOK) || (_modem == MODEM_FSK))
	{
		...
		// Waiting for the end of the transmission
		volatile uint8_t r = spi_rcv_data(REG_LR_IRQFLAGS);
		r = spi_rcv_data(REG_LR_IRQFLAGS);
		while ( !DIO0 );
        ...
        bool fixLen, bool crcOn )
{
	...
	//paConfig = ( paConfig & RF_PACONFIG_PASELECT_MASK ) | SX1276GetPaSelect( _channel); // MOD : Commented
	paConfig = paConfig | RF_PACONFIG_PASELECT_PABOOST; // MOD : Added
